...
Orthogonal lever: **[ponytail](https://github.com/DietrichGebert/ponytail)** cuts generated *code* volume (YAGNI-first ladder before writing anything new) rather than prose. Third-party plugins/marketplaces (ponytail, caveman, the workflow pack) now live in `templates/claude-code/settings.community-plugins.json` and are **opt-in only**: `PROJECT_BRAIN_COMMUNITY_PLUGINS=1 npm run brain:update-skill` (or `--community-plugins`) merges them; a plain sync never enables external code silently ([`decisions/0028`](.project-brain/decisions/0028-commercial-agent-ops-direction.md)). Audit marketplaces with `brain:skill-audit` before opting in. See [`references/conventions.md`](references/conventions.md) and [`decisions/0016`](.project-brain/decisions/0016-ecosystem-skill-axis-map.md) for why these are conventions, not npm dependencies.
