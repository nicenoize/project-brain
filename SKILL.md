...
**Recommended alternative model:** `Xenova/bge-small-en-v1.5` (also 384-dim, drop-in) measured **hit@8 0.810 → 0.857 and MRR 0.528 → 0.605** over the default MiniLM on the hard conceptual eval subset — the one change in this initiative that moved recall (where contextual prefixes and graph expansion did not). To adopt: `BRAIN_LOCAL_EMBED_MODEL=Xenova/bge-small-en-v1.5` then `npm run brain:index -- --force`. Not yet the default — validated on n=21; confirm on a larger hard eval set before flipping (see `docs/eval-methodology.md`).
