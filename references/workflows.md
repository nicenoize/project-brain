...
8. Add `--launch-runners --runner-cmd '...'` only when the user explicitly wants runner processes started. Runner command placeholders are shell-quoted: `{prompt}`, `{task}`, `{actor}`, `{tool}`, `{branch}`, `{issue}`, `{title}`, `{cwd}`, `{tier}`. Optional `--runner-cmd-light '...'` (or `BRAIN_RUNNER_CMD_LIGHT`) runs *small* issues on a cheaper runner (`tier=light`) and everything else on `--runner-cmd`. It is off unless set, and the tier is recorded per runner (`BRAIN_RUNNER_TIER`) so the two tiers' outcomes can be compared before anyone relies on it.
